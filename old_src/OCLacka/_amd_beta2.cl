...
__attribute__((reqd_work_group_size(1024, 1, 1)))
__kernel void FluffyRoundP(const __global uint2 * source, __global uint2 * destination, __global const int * sourceIndexes, __global int * destinationIndexes, int bktInSize, int bktOutSize)
{
	const int gid = get_global_id(0);
	const short lid = get_local_id(0);
	const int group = get_group_id(0);
	...
	const int expEdg = edgesInBucket / 4096 / 2;

	for (int i = 0; i < 4; i++)
	{
		ecounters[lid + (1024 * i)] = int bktIdx = min(atomic_inc(destinationIndexes + bucket), bktOutSize - 1);
	}
